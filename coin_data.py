...
def get_supply_change_last_year(name, current_supply):
    ...
    try:
        ...
        if price_1y == 0:
            return {"error": "Precio hace un año era 0. No se puede calcular el supply."}
...
def build_cripto_summary(cripto_name):
    ...
    # ⚠️ Validar si no hay datos o falta market_data
    if not cripto_info or "market_data" not in cripto_info:
        return {"error": "No se encontraron datos de mercado para esta moneda."}
